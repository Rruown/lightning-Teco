...
class SklearnDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(
            SklearnDataset(self.x_train, self.y_train,
                           self._x_type, self._y_type),
...
class ClassifDataModule(SklearnDataModule):
    def __init__(
        ...
        super().__init__(data, x_type=torch.float32,
                         y_type=torch.long, batch_size=batch_size)
...
class ClassificationModel(LightningModule):
    def __init__(self, num_features=32, num_classes=3, lr=0.01):
        ...
        self.train_acc = Accuracy(
            task="multiclass", num_classes=num_classes) if _TM_GE_0_11 else Accuracy()
