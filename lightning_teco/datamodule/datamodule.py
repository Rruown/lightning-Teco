...
from lightning_teco.utils.imports import (
    _SDAA_AVAILABLE,
...
if _SDAA_AVAILABLE:
    ...
    from lightning_teco.datamodule.dataloaders.resnet_media_pipe import MediaApiDataLoader

import lightning_teco.datamodule.utils
...
def load_data(traindir, valdir, train_transforms, val_transforms):  # type: ignore[no-untyped-def]
    ...
    if "imagenet" not in traindir.lower() and "ilsvrc2012" not in traindir.lower():
        raise ValueError("Teco dataloader only supports Imagenet dataset")
...
class SDAADataModule(LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if _SDAA_AVAILABLE:
            if lightning_teco.datamodule.utils.is_gaudi2():
    ...
    def train_dataloader(self):  # type: ignore[no-untyped-def]
        ...
        if self.drop_last and lightning_teco.datamodule.utils.is_gaudi():
