...
if module_available("lightning"):
    ...
    from lightning.core.optimizer import _init_optimizers_and_lr_schedulers
    from lightning.plugins.precision import PrecisionPlugin
    from lightning.trainer.states import TrainerFn
    from lightning.utilities import GradClipAlgorithmType
    from lightning.utilities.exceptions import MisconfigurationException
    from lightning.utilities.model_helpers import is_overridden
    from lightning.utilities.rank_zero import WarningCache, rank_zero_info, rank_zero_warn
    from lightning.utilities.types import LRSchedulerConfig
...
class SDAADeepSpeedStrategy(SDAADDPStrategy):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        os.environ["DEEPSPEED_USE_SDAA"] = "true"
        # TBD - remove these work arounds
        os.environ["PT_SDAA_LAZY_ACC_PAR_MODE"] = "0"
        os.environ["PT_SDAA_ENABLE_REFINE_DYNAMIC_SHAPES"] = "0"
    ...
    def _validate_config(self) -> None:
        ...
        if "autotuning" in self.config and self.config["autotuning"]["enabled"] is True:
            raise MisconfigurationException("SDAA DeepSpeed strategy doesn't support `autotuning`")
