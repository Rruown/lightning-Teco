...
from lightning_teco.utils.imports import _SDAA_SYNAPSE_GREATER_EQUAL_1_17_0
from lightning_teco.utils.resources import (
    _SDAA_AVAILABLE,
...
if module_available("lightning"):
    ...
    from lightning.plugins.precision import Precision
...
if _SDAA_AVAILABLE:
    ...
    if _INTEL_NEURAL_COMPRESSOR_AVAILABLE:
        ...
        MAXABS_MEASURE = str(
            importlib.resources.path("lightning_teco.plugins.quant_config.fp8", "maxabs_measure.json")
        )
        MAXABS_QUANT = str(
            importlib.resources.path("lightning_teco.plugins.quant_config.fp8", "maxabs_quant.json")
        )


class SDAAPrecisionPlugin(Precision):
    ...
    def __init__(
        ...
        if self.precision == "fp8":
            ...
            self.fp8_inference_available = (
                fp8_available and _INTEL_NEURAL_COMPRESSOR_AVAILABLE and _SDAA_SYNAPSE_GREATER_EQUAL_1_17_0
            )
            ...
            if not _INTEL_NEURAL_COMPRESSOR_AVAILABLE or not _SDAA_SYNAPSE_GREATER_EQUAL_1_17_0:
                rank_zero_info(
                    "FP8 inference not available."
                    f"Synapse version found: {_SDAA_SYNAPSE_GREATER_EQUAL_1_17_0}. Should be >= 1.17.09."
                    f" Neural Compressor available: {_INTEL_NEURAL_COMPRESSOR_AVAILABLE}. Should be True."
                )
    ...
    def convert_modules(
        ...
        """
        assert self.precision == "fp8", "SDAAPrecisionPlugin.convert_modules() should only be used with precision=`fp8`."
        if inference:
            if not _SDAA_SYNAPSE_GREATER_EQUAL_1_17_0:
                raise OSError("FP8 inference on SDAA requires SynapsesAI >= 1.17.0")
            if not _INTEL_NEURAL_COMPRESSOR_AVAILABLE:
                raise ModuleNotFoundError(
                    " neural compressor not found. " "Install it using `pip install neural-compressor`"
                )
